...
from my_model.CNN_GAN import discriminator, generator
...
if __name__ == '__main__':
    ...
    z_dimension = 599
    lr = 0.0003
    mission = 'CNN-GAN-00-uk-house1-wm'
    saver = saver(taskname=mission, flag=False, seeder=False)
    ...
    data = NILMDataset(r'D:\Research\NILM\dataset\uk_dale', 'wm', 'uk',  window_size, houses=[1])
    ...
    for epoch in range(num_epoch):
        ...
        for i, (real_load, real_label) in enumerate(dataloader):
            ...
            # =================train discriminator
            real_load = real_load.reshape(real_load.shape[0], window_size,1).cuda()
            ...
            z = torch.randn(real_load.shape[0], 1, z_dimension).cuda()
            fake_load = G(z)
            fake_load = fake_load.reshape(-1,599,1)
            ...
            while (flag):
                ...
                z = torch.randn(batch_size, 1, z_dimension).cuda()
                fake_load = G(z)
                fake_load.reshape(-1, 599, 1)
                output = D(fake_load)
        ...
        if (epoch + 1) % 20 == 0:
            for k in range(10):
                z = torch.randn(1, 1, z_dimension).cuda()
                fake_load = G(z)
